#' Create ECS Fault Displacement Measurements Input File from Esri Shapefile
#'
...
#' EPSG:4326, adds a default 'Rank' field if missing, and formats the data for
#' ECS tool input. The output is saved as a CSV file in the specified or default
#' directory. Note that a column called 'displ' that contains the fault
#' displacement measurements is required and execution is halted with an error
...
#'
#' @export
create_ecs_input_measurements <- function(in_filepath, output_folder = NULL) {
  ...
  # Analysis
  dataframe <- process_measurements(sf_object)

  # Set up output path and file name
  prefix <- "displacement_sites-"
